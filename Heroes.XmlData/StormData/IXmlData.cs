...
internal interface IXmlData
{
    ...
    StormElementData GetElementDataAt(ReadOnlySpan<char> index);
    ...
    /// <exception cref="KeyNotFoundException"><paramref name="index"/> was not found.</exception>
    StormElementData GetElementDataAt(string index);
    ...
    bool TryGetElementDataAt(ReadOnlySpan<char> index, [NotNullWhen(true)] out StormElementData? stormElementData);
    ...
    /// <returns><see langword="true"/> if the index is found, otherwise <see langword="false"/>.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="index"/> is <see langword="null"/>.</exception>
    bool TryGetElementDataAt(string index, [NotNullWhen(true)] out StormElementData? stormElementData);
    ...
    IEnumerable<StormElementData> GetAllElementData();
}
